...
def sort_data(dataframe):
	...
	dataframe.sort_values("OS_MONTHS", ascending = False, inplace = True)
	
	x = dataframe.drop(["SAMPLE_ID", "OS_MONTHS", "OS_EVENT", "AGE"], axis = 1).values
	ytime = dataframe.loc[:, ["OS_MONTHS"]].values
	yevent = dataframe.loc[:, ["OS_EVENT"]].values
	age = dataframe.loc[:, ["AGE"]].values

	return(x, ytime, yevent, age)


def load_data(dataframe, dtype):
	...
	'''
	x, ytime, yevent, age = sort_data(dataframe)
